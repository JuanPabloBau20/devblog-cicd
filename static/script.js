...
function initializeFormValidation() {
    ...
    if (createPostForm) {
        ...
        createPostForm.addEventListener('submit', function (e) {
            if (!validateForm()) {
                e.preventDefault(); showAlert('Por favor corrige los errores antes de continuar', 'danger');
}
        });
    }
}
...
function updateCharacterCount(inputId, maxLength) {
    ...
    let counter = document.getElementById(inputId + '-counter'); if (!counter) {
...
function initializeAnimations() {
    ...
    }, { threshold: 0.1 }); cards.forEach(card => {
...
<!DOCTYPE html>
<html>
<head><title>Vista Previa - ${title}</title>
<link
